...
describe('First Aid (e2e)', () => {
  ...
  describe('POST /api/first-aid/advice', () => {
    it('정상 요청 → 201, AI 조언 및 응급연락처 반환', async () => {
      ...
      expect(res.body.message).toBe('성공');
      expect(res.body.data.content).toBeTruthy();
      expect(res.body.data.summary).toBeTruthy();
      expect(res.body.data.recommendedAction).toBeTruthy();
      expect(res.body.data.confidence).toBeDefined();
      expect(res.body.data.identificationResponse.countryCode).toBe('KR');
      expect(res.body.data.identificationResponse.emergencyContact).toBeDefined();
    });
