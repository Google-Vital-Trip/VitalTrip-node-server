...
describe('Encyclopedia (e2e)', () => {
      ...
      data: [
        {
          ...
          fullSummary: 'Burns are injuries to skin caused by heat, chemicals, or radiation.',
        ...
        },
        {
          ...
          fullSummary: 'Bleeding is the loss of blood from the circulatory system.',
          categories: '["Injuries and Wounds"]',
  ...
  describe('GET /api/encyclopedia', () => {
    it('전체 조회 → total/items 반환', async () => {
      const res = await request(app.getHttpServer()).get('/api/encyclopedia');
    ...
    it('키워드 검색 → 일치하는 항목만 반환', async () => {
      ...
      expect(res.body.data.items[0].title).toBe('Burns');
    });
    ...
    it('altTitles가 배열로 파싱되어 반환', async () => {
      ...
      expect(Array.isArray(res.body.data.items[0].altTitles)).toBe(true);
      expect(res.body.data.items[0].altTitles).toContain('Chemical Burns');
    });

    it('categories가 배열로 파싱되어 반환', async () => {
      const res = await request(app.getHttpServer()).get('/api/encyclopedia');

      expect(Array.isArray(res.body.data.items[0].categories)).toBe(true);
    });
